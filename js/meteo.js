...
const handleVoice = (text) => {
    ...
    // "Météo à Rennes" = ["météo à", "Rennes"]
    const handledText = text.toLowerCase();
    if(handledText.includes('météo à')){
        const location = handledText.split('à')[1].trim();
    ...
    // 'changer couleur de fond [Black, white, Orange, Yellow, ... ]'
    const container = document.querySelector('.container');
    if(handledText.includes('changer couleur de font')) {
        const color = handledText.split('fond')[1].trim();  
        container.style.background = color;
        return;
    }
    if(handledText.includes('couleur de font default')){
        container.style.background = '';
        return;
    }

    if(handledText.includes('quelle heure est-il')){
